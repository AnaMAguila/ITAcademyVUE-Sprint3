...
window.addEventListener('load', ()=> {
	...
	// Newsletter
	var formNewslleter = document.querySelector('.form-newsletter');
	var sEmail = document.getElementById("sEmail");

	formNewslleter.addEventListener('submit', (e) => {
		e.preventDefault();
		validateNewsletter();
	})
	...
	function validateNewsletter() {
		...
		if(sEmail.value.length < 3 || !validEmail.test(sEmail.value)){
			sEmail.classList.remove('is-valid');
			sEmail.classList.add('is-invalid');
		}else{
			sEmail.classList.remove('is-invalid');
			sEmail.classList.add('is-valid');
		}
	}
})
